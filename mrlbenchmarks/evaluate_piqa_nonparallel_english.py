...
shared stem). So the model picks the correct full statement of the two.

Method (see common.py docstring): few-shot, 2-way MCQ, scored by highest
next-token probability among the single-token option letters " A"/" B" after
"Answer:". Few-shot exemplars are a BALANCED set (4 each of A/B for the default
8-shot) held out from the test set. base/CPT/llama scored raw; the instruct
model (--alpaca-models) is re-wrapped in the Alpaca template. Runs on GPU
(bf16) by default; --device cpu (float32) for a no-GPU fallback.
"""
...
def build_records(rows, kshot, seed):
    ...
    shot_idx = C.pick_fewshot(golds, per_key, seed=seed)
    ...
    for i in shot_idx:
        ...
        shot_blocks.append(C.render_en_nonparallel(choices,
    ...
    for i, row in enumerate(rows):
        ...
        test_block = C.render_en_nonparallel(choices, answer_letter=None)
...
def main():
    ...
    ap.add_argument("--kshot", type=int, default=8, help="# balanced few-shot exemplars (held out)")
    ...
    ap.add_argument("--quant", choices=["bf16", "4bit", "8bit"], default="bf16",
                    help="only used when --device cuda")
    ...
    prec = "fp32" if args.device == "cpu" else args.quant
    ...
    gpu = "CPU" if args.device == "cpu" else (
    ...
    def make_meta():
        return dict(bench="Global-PIQA non-parallel (English)", gpu=gpu, total=n_valid,
                    quant=prec, kshot=args.kshot, n_shots=len(shot_idx))
    ...
    for path in args.models:
        ...
        if args.skip_existing and os.path.exists(mpath):
            ...
            print(f"\n=== Evaluating {name}  (prompt format: {fmt}, {args.kshot}-shot, {prec}) ===")
            t0 = time.time()
            model, tok = C.load_model(path, args.quant, args.device)
