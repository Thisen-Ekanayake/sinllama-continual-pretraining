...
Method (see common.py docstring for full detail): few-shot, 4-way MCQ, scored
by highest next-token probability among digits "1".."4" after the Sinhala
"පිළිතුර:" cue. Few-shot exemplars are a BALANCED set (equal count per gold
answer key — 2 each of 1/2/3/4 for the default 8-shot) held out from the test
set, to break the strong first-option bias base/CPT models show in zero-shot.
base/CPT/llama scored raw; the instruct model (--alpaca-models) is re-wrapped
in the Alpaca template it was SFT'd on. Runs on GPU (bf16) by
default; --device cpu (float32) for a no-GPU fallback.
"""
