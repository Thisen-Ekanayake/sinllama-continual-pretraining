#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# Run all 4 Global-PIQA few-shot eval scripts on a GPU cloud pod (bf16) —
# Sinhala first (both datasets), then English (both datasets) — then scp each
# results_* directory back to this local mrlbenchmarks/.
#
# Each script holds out 8 balanced few-shot exemplars, excludes them from the
# test set, and loads each model in bf16 on the pod GPU (--device cuda).
#
...
KSHOT="${KSHOT:-8}"
EVAL_ARGS="--device cuda --quant bf16 --kshot $KSHOT --batch-size $BATCH_SIZE --max-len $MAX_LEN"
...
echo "[$(date '+%F %T')] Running $TOTAL few-shot eval scripts on $POD_HOST (one at a time, bf16 GPU)"
