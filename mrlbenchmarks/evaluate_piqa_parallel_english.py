...
Method (see common.py docstring): few-shot, 4-way MCQ, scored by highest
next-token probability among the single-token option letters " A".." D" after
"Answer:". Few-shot exemplars are a BALANCED set (2 each of A/B/C/D for the
default 8-shot) held out from the test set. base/CPT/llama scored raw; the
instruct model (--alpaca-models) is re-wrapped in the Alpaca template it was
SFT'd on. Runs on GPU (bf16) by default; --device cpu (float32) as a fallback.
"""
