"""
LoRA SFT finetuning of the merged SinLlama model on the
Sinhala-News-Category-classification task.

Dataset: data/news/news_{train,val,test}.csv
  columns: "comments", "labels", "length"
  labels: numeric 0-4  (Political:0, Business:1, Technology:2, Sports:3, Entertainment:4)
...
Example:
    python finetune/finetune_news_category.py \
        --model_name_or_path models/SinLlama_cpt_merged \
        --data_dir data/news \
        --output_dir runs/news_category_lora
"""
import argparse
import os
...
def parse_prediction(generated_text: str):
    for ch in generated_text.strip():
        if ch in LABELS:
            return ch
    return None
...
def load_split(data_dir, name):
    ...
    df[LABEL_COL] = df[LABEL_COL].apply(lambda v: str(int(float(str(v).strip()))))
...
def write_results_report(path, args, train_result, trainer, metrics, samples):
    ...
    for lab in LABELS:
        ...
        lines.append(f"    label {str(lab):4s} {m['precision']:10.4f} {m['recall']:10.4f} "
...
def parse_args():
    ...
    p.add_argument("--data_dir", default="data/news")
    p.add_argument("--train_file", default="news_train.csv")
    p.add_argument("--val_file", default="news_val.csv")
    p.add_argument("--test_file", default="news_test.csv")
    p.add_argument("--output_dir", default="runs/news_category_lora")
    p.add_argument("--results_file", default=None,
                   help="Where to write the final report (default: <output_dir>/results_news.txt).")
    ...
    p.add_argument("--run_name", default="news-category-lora")
    return p.parse_args()
...
def main():
    ...
    for lab in LABELS:
        m = metrics["per_label"][lab]
        print(f"  label {str(lab):4s} P={m['precision']:.4f} R={m['recall']:.4f} "
