#!/bin/bash
# Downstream LoRA finetune: Sinhala News-Category classification.
#   labels: 0..4                              (train=2596)
#
# Writes results_news.txt into the output_dir.
#
# Usage:
#   bash finetune/run_news_category.sh
#   FAST_LOAD=0 bash finetune/run_news_category.sh   # skip the /dev/shm copy
...
$PY "${SCRIPT_DIR}/finetune_news_category.py" \
    ...
    --results_file "${OUTPUT_DIR}/results_news.txt" \
    ...
    --run_name "${RUN_NAME}" \
    $(fourbit_flag) \
    || { echo "!!! news-category FAILED"; exit 1; }

echo "Done. Results -> ${OUTPUT_DIR}/results_news.txt"
