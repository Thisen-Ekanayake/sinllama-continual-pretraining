...
Sentiment-Analysis (sentiment-tagger) task.

Dataset: data/sentiment/sentiment_{train,val,test}.csv
...
Example:
    python finetune/finetune_sentiment.py \
...
# ----------------------------------------------------------------------------
TASK = "sentiment"
TEXT_COL = "phrase"          # the dataset stores the text in "phrase"
LABEL_COL = "sentiment"
LABELS = ["POSITIVE", "NEGATIVE", "NEUTRAL"]
...
class SFTDataset(Dataset):
    ...
    def __init__(self, df, tokenizer, max_seq_length):
        ...
        for text, label in zip(df[TEXT_COL].tolist(), df[LABEL_COL].tolist()):
            ...
            budget = max(budget, 0)
            text_ids = tokenizer(str(text), add_special_tokens=False)["input_ids"][:budget]

            prompt_ids = [bos] + prefix_ids + text_ids + suffix_ids
...
def evaluate_accuracy(model, tokenizer, df, max_seq_length, batch_size, device):
    ...
    for start in range(0, len(texts), batch_size):
        ...
        for text in texts[start:start + batch_size]:
            ...
            text_ids = tokenizer(str(text), add_special_tokens=False)["input_ids"][:max(budget, 0)]
            batch_prompts.append([bos] + prefix_ids + text_ids + suffix_ids)
...
def parse_args():
    ...
    p.add_argument("--data_dir", default="data/sentiment")
    p.add_argument("--train_file", default="sentiment_train.csv")
    p.add_argument("--val_file", default="sentiment_val.csv")
    p.add_argument("--test_file", default="sentiment_test.csv")
    p.add_argument("--output_dir", default="runs/sentiment_lora")
    p.add_argument("--results_file", default=None,
                   help="Where to write the final report (default: <output_dir>/results_sentiment.txt).")
    ...
    p.add_argument("--run_name", default="sentiment-lora")
    return p.parse_args()
