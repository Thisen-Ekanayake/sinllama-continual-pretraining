#!/bin/bash
# Downstream LoRA finetune: Sinhala Writing-Style classification.
#   labels: ACADEMIC, CREATIVE, NEWS, BLOG   (train=10010)
#
# Writes results_writing.txt into the output_dir.
#
# Usage:
#   bash finetune/run_writing_style.sh
#   FAST_LOAD=0 bash finetune/run_writing_style.sh   # skip the /dev/shm copy
...
TRAIN_FILE=writing_style_train.csv
VAL_FILE=writing_style_val.csv
TEST_FILE=writing_style_test.csv
MAX_SEQ_LEN=512
LORA_RANK=16
LORA_ALPHA=32
LORA_DROPOUT=0.05
...
$PY "${SCRIPT_DIR}/finetune_writing_style.py" \
    ...
    --results_file "${OUTPUT_DIR}/results_writing.txt" \
    ...
    --run_name "${RUN_NAME}" \
    $(fourbit_flag) \
    || { echo "!!! writing-style FAILED"; exit 1; }

echo "Done. Results -> ${OUTPUT_DIR}/results_writing.txt"
