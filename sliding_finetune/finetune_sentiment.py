"""
SinLlama LoRA Fine-tuning — Sentiment Classification (POSITIVE / NEGATIVE)
===========================================================================

Methods ported from sentiment_context_aware_finetune.py (same as news script):
...
Dataset expected at:
  data_new/sentiment/{train,val,test}.jsonl
  Each line: {"text": "...", "label": "POSITIVE"} or "NEGATIVE"
"""
...
DATA_DIR   = os.environ.get("DATA_DIR",   "data_new/sentiment")
OUT_DIR    = os.environ.get("OUT_DIR",    "outputs/sentiment_lora")
...
# Normalise raw dataset labels ("POSITIVE"/"NEGATIVE") to lowercase
RAW_TO_NORM = {"POSITIVE": "positive", "NEGATIVE": "negative"}
NORM_TO_ID  = {"positive": 0, "negative": 1}
...
print("=" * 70)
print("SinLlama — SENTIMENT CLASSIFICATION  (LoRA, sliding-window)")
print("=" * 70)
...
def build_prompt(text: str, label_name: str) -> str:
    ...
    return (
        f"Does the following Sinhala text have a POSITIVE or NEGATIVE sentiment?\n\n"
        f"Text: {text}\n"
        f"Sentiment: {label_name}{tokenizer.eos_token}"
    )
...
# At inference we score only these two token positions so the model
...
if USE_WANDB:
    run = wandb.init(
        project="sinllama-sentiment-classification",
        name=f"sentiment_lora_lr{LR}_r{LORA_R}_ep{int(EPOCHS)}",
        resume="allow",
        config={
            "model":          MODEL_PATH,
...
training_args = TrainingArguments(
    output_dir=OUT_DIR,
    run_name=f"sentiment_lora_lr{LR}_r{LORA_R}",
    bf16=True,
    tf32=True,
